...
Alegreya gives headings the handled character of an inspection handbook. It is upright, medium weight and naturally spaced, so it does not turn the page into a magazine cover. Mulish keeps instructions and controls readable at working sizes. The sealed teaching diagrams retain their reviewed Roboto Slab and Mulish annotation system. Corners stay modest. Images use the existing 16:9 CourseLit renderer with a small radius, no border and no shadow. The design uses no gradients, chatbot imagery, new block or custom CSS.
...
The short fact line earns the managed marker's visible place without freezing curriculum counts. The next section starts with three recognisable kinds of work, then explains when chat, a reusable skill or a controlled connection fits. A vertical diagram carries the visual explanation; the adjacent ordered list supplies the same decision path in text.
