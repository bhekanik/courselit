...
- Section group IDs: `group_ai_for_actual_work_01` through `group_ai_for_actual_work_07`, with ranks in thousands from 1000 to 7000.
- Lesson IDs: `lesson_ai_for_actual_work_01` through `lesson_ai_for_actual_work_14`, numbered across the whole course rather than within a section.
...
The manifest carries top-level `course.capstone` metadata holding the key, the stable ID, the title, the outcome, the exercise, the ten artefacts and the seven assessment checks. That is the machine-readable record and the thing an assessment surface should read.

The learner never sees a fifteenth lesson. The run instructions, the ten submission artefacts and the seven assessment checks are written into lesson 14 under a heading named exactly `Capstone: run your workflow`. The `embeddedInLessonKey` field points at `turn-misses-into-system-improvements` to record that placement.
...
The source notes are engineering-heavy and support the mechanism, evidence, delivery and understanding arguments well. They do not contain worked non-coding cases, a privacy and permission procedure, a current tool comparison, non-code verification examples or an assessment rubric.
...
The verifier fails the build on private paths, learner-facing URLs and named vendors, so this boundary is enforced rather than trusted.
...
- Any future lesson on cost, provider access or integrations must be written from live verification and carry its own review date. None exists yet, and the notes do not support one.

Reviewing means rerunning the observation, not rereading the prose.

## Assessment caveat

The ten artefacts and the seven assessment checks are a starting rubric, not a validated one. They have not been piloted against real submissions.

Before the assessment is used to pass or fail anybody, pilot it against deliberately weak submissions and confirm that plausible-looking but unsupported work actually fails. The known risk is that a learner produces fourteen well-formed templates with no completed run, which the rubric is meant to catch and has not yet been proven to catch.
...
- TDD: the acceptance checks and the verifier were authored and locked before `course.json` existed, so the suite ran red on a missing manifest first. Content was then added in slices with the verifier rerun after each. Note honestly that the intermediate slices could not pass, because the verifier hard-requires seven sections, fourteen lessons, a capstone and at least four extensions from the start. Until this change, `source-map.md` and `editorial-notes.md` were still failing their existence checks.
...
- TypeScript reviewer: not applicable to the implementation. This phase changes JSON, Markdown and one standalone JavaScript verifier only. It remains applicable as a negative check, and the check passes: no TypeScript contract, type declaration, package manifest or build configuration entered the diff.
...
- The recurring spine across notes written for different projects is context, mechanism choice, evidence, guardrails, delivery state and deliberate understanding. The seven-section order follows it and each section consumes the artefacts from the one before.
...
- Fourteen artefacts can become paperwork. Each one is tied to the single real workflow and checked against actual output, and that tie is the thing most likely to erode.
